		...
		<div class="category_mega_menu_wrap" style="<?php echo esc_attr( $style ) ?>">
			...
			<ul class="list-unstyled sub_category_mega_menu">
				...
				foreach( $subterms as $subterm ){
					?>
					<li><a href="<?php echo esc_url( compare_add_query_arg( array( $compare_slugs['product-cat'] => $subterm->slug ), $COMPARE_SEARCH_URL ) ) ?>"><?php echo esc_html( $subterm->name ); ?></a></li>
					<?php
				}
				?>
			</ul>
	...
	if( !empty( $categories ) ){
		foreach( $categories as $category ){
			...
			if( in_array( $category->term_id, $ancestors ) ){
				$children = get_terms( 'product-cat', array( 'parent' => $category->term_id, 'hide_empty' => false, ) );
				echo '<ul class="list-unstyled">';
...
function compare_import_cf_values(){
	global $wpdb;
	$tables = array(
		'stores' => array( '%d', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%s' ),
	...
	if( !empty( $_POST['compare_custom_data'] ) ){
		...
		if( !empty( $compare_custom_data ) ){
			...
			<div class="updated notice notice-success is-dismissible">
				...
				<button type="button" class="notice-dismiss"><span class="screen-reader-text"><?php esc_html_e( 'Dismiss this notice.', 'compare' ) ?></span></button>
			</div>
			<?php
		}
	}
