<?php
	...
	function compare_add_feed_row( $product_id, $store_id, $url, $price, $shipping, $shipping_comment, $stock ){
		global $wpdb;
		$wpdb->get_results(
			$wpdb->prepare(
	...
	*/
	...
	function compare_core_import( $name, $description, $categories, $tags, $brand, $image, $pid, $short_desc, $url, $price, $shipping, $shipping_comment, $stock, $store_id, $cat_separator = ',', $tag_separator = ',' ){
		if( !empty( $name ) ){
			if( !empty( $categories ) ){
				if( !empty( $brand ) ){
					if( !empty( $price ) ){
						if( !empty( $pid ) ){
							...
							/* add to feed list */
							compare_add_feed_row( $product_id, $store_id, $url, $price, $shipping, $shipping_comment, $stock );
	...
	function compare_store_parse_and_import( $link, $store_id, $store_custom_parser ){
		...
		if( empty( $store_custom_parser ) ){
			foreach ( $xml->xpath('/products/product') as $product ) {
				...
				$stock = !empty( $product->stock ) ? trim( (string)$product->shipping ) : '';
				$shipping_comment = !empty( $product->shipping_comment ) ? trim( (string)$product->shipping_comment ) : '';

				compare_core_import( $name, $description, $categories, $tags, $brand, $image, $pid, $short_desc, $url, $price, $shipping, $shipping_comment, $stock, $store_id );
			...
			$parser_stock = get_post_meta( $store_custom_parser, 'parser_stock', true );
			...
			foreach ( $xml->xpath('/'.$parser_product_root ) as $product ) {
				...
				$stock = $product->xpath('./'.$parser_stock);
				$stock = !empty( $stock ) ? trim( (string)$stock[0] ) : '';
				...
				compare_core_import( $name, $description, $categories, $tags, $brand, $image, $pid, $short_desc, $url, $price, $shipping, $shipping_comment, $stock, $store_id, $parser_cats_separator, $parser_tags_separator );
