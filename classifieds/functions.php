...
function classifieds_requred_plugins() {
	$plugins = array(
		...
		array(
			'name'               => esc_html__( 'Social Login', 'classifieds' ),
			'slug'               => 'accesspress-social-login-lite',
			'source'             => get_template_directory() . '/lib/plugins/accesspress-social-login-lite.zip',
			'required'           => true,
			'version'            => '',
			'force_activation'   => false,
			'force_deactivation' => false,
...
/*
Check version of the theme and update if necessary
*/
...
function classifieds_ad_status_meta_box() {
	add_meta_box( 'classifieds_ad_status', esc_html__( 'Ad Status', 'classifieds' ), 'classifieds_ad_status', 'ad', 'side', 'high' );
}
...
function classifieds_export_cf_values() {
	...
	if ( ! empty( $cfs ) ) {
		...
		foreach ( $cfs as $cf ) {
			...
			$results                   = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}custom_fields WHERE post_id = %d", $cf->ID ) );
		...
		if ( ! empty( $cf_values ) ) {
				...
				foreach ( $custom_fields as $row ) {
					...
					if ( ! empty( $row['fields'] ) ) {
						foreach ( $row['fields'] as $field_row ) {
							$info = $wpdb->query( $wpdb->prepare( "INSERT INTO {$wpdb->prefix}custom_fields VALUES( '', %d, %s, %s, %s, %s, %s, %d )", $post_id, $field_row['name'], $field_row['label'], $field_row['type'], $field_row['field_values'], $field_row['child_of_value'], $field_row['parent'] ) );
						}
					}
				}
			}
			...
			if ( ! empty( $custom_fields_meta ) ) {
				foreach ( $custom_fields_meta as $row ) {
					$info = $wpdb->query( $wpdb->prepare( "INSERT INTO {$wpdb->prefix}custom_fields_meta VALUES( '', %d, %s, %s )", $row['post_id'], $row['name'], $row['val'] ) );
				}
			}
			...
			if ( ! empty( $info ) ) {
				...
				<div class="updated notice notice-success is-dismissible">
					...
					<button type="button" class="notice-dismiss"><span
							class="screen-reader-text"><?php esc_html_e( 'Dismiss this notice.', 'classifieds' ) ?></span>
					</button>
				</div>
				<?php
			} else {
...
function classifieds_add_post_meta( $post_id, $meta_key, $meta_value ) {
	global $wpdb;
	$return = $wpdb->insert( $wpdb->prefix . 'custom_fields_meta', array(
		'post_id' => $post_id,
		'name'    => $meta_key,
		'val'     => $meta_value
	), array(
...
/*
Get URL based on the template which is being used
*/
...
/*
generate random hash for the register
*/
			...
			<td>
				<select name="user_active_status" id="user_active_status">
					<option <?php echo ! empty( $user_active_status ) && $user_active_status != 'active' ? 'selected="selected"' : '' ?>
						value="inactive"><?php esc_html_e( 'Inactive', 'classifieds' ) ?></option>
					<option <?php echo empty( $user_active_status ) || $user_active_status == 'active' ? 'selected="selected"' : '' ?>
						value="active"><?php esc_html_e( 'Active', 'classifieds' ) ?></option>
				</select>
			</td>
		</tr>
		<tr>
			...
			<td>
				<select name="is_verified" id="is_verified">
					<option <?php echo ! $is_verified ? esc_attr__( 'selected="selected"' ) : '' ?>
						value="no"><?php esc_html_e( 'No', 'classifieds' ) ?></option>
					<option <?php echo $is_verified ? esc_attr__( 'selected="selected"' ) : '' ?>
						value="yes"><?php esc_html_e( 'Yes', 'classifieds' ) ?></option>
				</select>
			</td>
		</tr>
		<tr>
		...
		<tr>
			...
			<td>
				<input type="text" name="phone_number" id="phone_number"
				       value="<?php echo esc_attr__( $phone_number ) ?>"/>
			</td>
		</tr>
		<tr>
			...
			<td>
				...
				<input type="hidden" name="cover_image" id="cover_image"
				       value="<?php echo esc_attr__( $cover_image ) ?>" class="form-control">
			</td>
		</tr>
		<tr>
...
function classifieds_scripts_styles() {
	...
	wp_enqueue_script( 'classifieds-googlemap', $protocol . '://maps.googleapis.com/maps/api/js?key=' . classifieds_get_option( 'map_api' ) . '&libraries=places', array( 'jquery' ), false, true );
...
/*
Load resources which are available in the admin backend
*/
...
/*
Add admin ajax url on the page
*/
...
function classifieds_get_custom_fields( $post_id ) {
	...
	$fields = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}custom_fields WHERE post_id = %d AND parent = 0 ORDER BY field_id", $post_id ) );
...
function classifieds_get_custom_subfields( $post_id, $field_id ) {
	...
	$subfields = $wpdb->get_results( $wpdb->prepare( "SELECT * FROM {$wpdb->prefix}custom_fields WHERE post_id = %d AND parent = %d ORDER BY field_id", $post_id, $field_id ) );
...
function classifieds_add_meta_box_custom_fields() {
	add_meta_box( 'classifieds_custom_meta', esc_html__( 'Custom Fields Data', 'classifieds' ), 'classifieds_populate_meta_box_custom_fields', 'custom_field' );
}
...
/*
Populate meta box for the custom fields custom post type
*/
...
function classifieds_save_custom_fields_meta( $post_id ) {
	...
	foreach ( $_POST['cf_field'] as $key => $field ) {
		if ( $key !== 'x' ) {
			$wpdb->query( $wpdb->prepare( "INSERT INTO {$wpdb->prefix}custom_fields VALUES( '', %d, %s, %s, %s, %s, %s, %d )", $post_id, $field['cf_name'], $field['cf_label'], $field['cf_type'], $field['cf_select_values'], '', 0 ) );

			if ( $field['has_subgroups'] == '1' ) {
				$parent = $wpdb->insert_id;
				foreach ( $field['cf_subfield'] as $subkey => $subvalues ) {
					if ( $subkey !== 'valuex' ) {
						$wpdb->query( $wpdb->prepare( "INSERT INTO {$wpdb->prefix}custom_fields VALUES( '', %d, %s, %s, %s, %s, %s, %d )", $post_id, $field['cf_subfield_name'], $field['cf_subfield_label'], 'select', $subvalues, $subkey, $parent ) );
					}
				}
			}
...
function classifieds_delete_custom_fields_meta( $post_id ) {
	...
	if ( $post_type == 'custom_field' ) {
		...
		$wpdb->query( $wpdb->prepare( "DELETE postmeta.* FROM {$wpdb->prefix}custom_fields AS cf LEFT JOIN {$wpdb->prefix}custom_fields_meta AS postmeta ON cf.name = postmeta.name WHERE cf.post_id = %d", $post_id ) );
		...
		if ( filter_var( $email, FILTER_VALIDATE_EMAIL ) ) {
			...
			if ( ! empty( $chimp_api ) && ! empty( $chimp_list_id ) ) {
				...
				if ( $result === false ) {
					...
					$message = '<div class="alert alert-danger">' . json_encode( $result['error'] ) . '</div>';
				} else {
...
/*
Add video container around embed videos
*/
...
/* generate nice permalink structure */
...
function classifieds_generate_marker_info( $args, $check_cache = false ) {
	...
	if ( $check_cache && get_transient( 'classifieds_home_markers' ) ) {
		...
		while ( $has_rows ) {
			...
			$markers = $wpdb->get_results( $wpdb->prepare( "SELECT postmeta1.meta_value AS longitude, postmeta2.meta_value AS latitude, post_title, ID, post_author FROM {$wpdb->posts} AS posts 
	    			...
	    			GROUP BY posts.ID LIMIT %d OFFSET %d", current_time( 'timestamp' ), $limit, $offset ) );
...
function classifieds_update_ad() {
	$ad_approval   = classifieds_get_option( 'ads_approval' );
				...
				if ( ! empty( $ad_gmap_longitude ) && ! empty( $ad_gmap_longitude ) ) {
					if ( ! empty( $ad_category ) ) {
						$post_args = array(
							...
							'post_status'  => ( $ad_approval === 'yes' ) ? 'draft' : 'publish',
						);
						...
						if ( $_POST['post_id'] == 0 ) {
							...
							if ( empty( $amount ) ) {
								...
								if ( 'no' == $ad_approval ) {
									$response = '<div class="alert alert-success">' . esc_html__( 'Your ad has been added', 'classifieds' ) . '</div>';
								} else {
...
function classifieds_update_parent( $post_parent_id, $post_id ) {
	...
	$post_args   = array(
		...
		'post_status'   => ( 'yes' == $ad_approval ) ? 'draft' : 'publish',
	);

	if ( 'yes' == $ad_approval ) {
		wp_update_post( $post_args );
		classifieds_inform_admin( $post_parent_id, $is_expired );
	} else {
		wp_update_post( $post_args );
	}
...
function classifieds_payment_result() {
	...
	switch ( $payment ) {
		case 'paypal' :
			$response = classifieds_pay_with_paypal();
			break;
		case 'payu' :
			$response = classifieds_pay_with_payu();
			break;
		case 'skrill' :
			$response = classifieds_pay_with_skrill();
			break;
		case 'skrill-verify' :
			$response = classifieds_skrill_payment_confirmation();
			break;
		case 'bank' :
			$response = classifieds_pay_with_bank();
			break;
		case 'ideal' :
			$response = classifieds_pay_with_ideal();
			break;
		case 'ideal-verify' :
			$response = classifieds_ideal_payment_confirmation();
			break;
		default :
			$response = '';
	}
		...
		if ( ! empty( $amount ) ) {
			$paypal = new PayPal( array(
				...
				'cancelUrl' => add_query_arg( array(
					'cancel' => 'yes',
					'ad_id'  => $ad_id
				), classifieds_get_permalink_by_tpl( 'page-tpl_register_store' ) ),
...
function classifieds_inform_admin( $post_id, $is_expired = false ) {
	...
	if ( $is_expired ) {
		$message = esc_html__( 'Ad submitted for renewal.', 'classifieds' );
		$subject = esc_html__( 'Ad Renewal Submitted', 'classifieds' );
		delete_post_meta( $post_id, 'ad_expire_mark' );
	} else {
		$message = esc_html__( 'New ad has been submitted.', 'classifieds' );
		$subject = esc_html__( 'New Ad Submitted', 'classifieds' );
	}
	$message .= '<br/><br/>';
	if ( 'yes' == $ad_approval ) {
		$message .= esc_html__( 'You can access it on the following link: ', 'classifieds' ) . esc_url( admin_url( 'post.php?post=' . $post_id . '&action=edit' ) );
	} else {
		$message .= esc_html__( 'You can see it on the following link: ', 'classifieds' ) . esc_url( get_permalink( $post_id ) );
	}
...
function classifieds_get_organized( $taxonomy ) {
	...
	if ( $sort == 'asc' ) {
		switch ( $sortby ) {
			case 'name' :
				usort( $taxonomy_organized, "classifieds_organized_sort_name_asc" );
				break;
			case 'slug' :
				usort( $taxonomy_organized, "classifieds_organized_sort_slug_asc" );
				break;
			case 'count' :
				usort( $taxonomy_organized, "classifieds_organized_sort_count_asc" );
				break;
			default :
				usort( $taxonomy_organized, "classifieds_organized_sort_name_asc" );
				break;
		...
		switch ( $sortby ) {
			case 'name' :
				usort( $taxonomy_organized, "classifieds_organized_sort_name_desc" );
				break;
			case 'slug' :
				usort( $taxonomy_organized, "classifieds_organized_sort_slug_desc" );
				break;
			case 'count' :
				usort( $taxonomy_organized, "classifieds_organized_sort_count_desc" );
				break;
			default :
				usort( $taxonomy_organized, "classifieds_organized_sort_name_desc" );
				break;
	...
	if ( $send_expire_notice == 'yes' ) {
		if ( ( current_time( 'timestamp' ) - $last_check ) >= 86400 ) {
			...
			while ( $has_data ) {
				...
				$counter ++;
				$results = $wpdb->get_results( $wpdb->prepare( "SELECT posts.ID AS post_id, user_login, user_email, post_title FROM {$wpdb->posts} AS posts 
		        		...
		        		GROUP BY posts.ID  LIMIT 500 OFFSET %d", current_time( 'timestamp' ), $offset ) );
				if ( ! empty( $results ) ) {
